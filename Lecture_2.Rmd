...
where $x_i$ is a p-dimensional vector of regressors for observation $i$, $\beta$ a p-dimenisional coefficient vector, and $\varepsilon_i$ iid error terms with $\mathbb{E} [\varepsilon_i|x_i]= 0$.
...
Unter homoscedastic errors, i.e. $\mathbb{Var} \varepsilon_i=\sigma^2$, we have that

$$ \mathbb{Var}(\hat{\beta})= (X'X)^{-1} \sigma^2.$$
...
* For the extensions we consider without loss of generality univariate regressions: $y_i=\beta_0 + \beta_1 x_i + \varepsilon_i$
...
* Estimation by ols
* Quite flexibel, but usually $p=3$ oder $p=4$ 
* Higher order polynomials ($p \geqq 5 $) might lead to strange fits (overfitting), especially at the boundary.
...
* Defintion: Step functions are functions which are constant on each part of a partition of the domain.
* Univariate Regression: choosing $K$ cutpoints $c_1, \ldots, c_K$ and defining new auxiliary variables:  
$C_0(x)= 1(x < c_1)$, $C_1(x)=1(c_1 \leq x < c_2)$, ..., $C_K(x)=1(c_K \leq x)$
